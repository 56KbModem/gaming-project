...
tf::gui::main_menu::main_menu(sf::RenderWindow & window):
window(window),
active_menu(menus::main)
{
    if(!menu_sound.openFromFile("assets/sounds/menu_sound.wav")){
        spdlog::get("stderr")->error("Failed to load audio file \"sounds/menu_sound.wav\"");
    }
    if(!selection_buffer.loadFromFile("assets/sounds/ui_select.wav")){
        spdlog::get("stderr")->error("Failed to load audio file \"sounds/ui_select.wav\"");
    }
    selection_music.setBuffer(selection_buffer);
}

game_mode tf::gui::main_menu::run() {
    ...
    action actions[] = {
            action( sf::Keyboard::Up,
                    ...
                    #if DEBUG
                    spdlog::get("console")->info("Move up");
                    #endif
            ...
            action( sf::Keyboard::Down,
                    ...
                    #if DEBUG
                    spdlog::get("console")->info("Move down");
                    #endif
                    sf::sleep( sf::milliseconds(200));}),
    };
    while (window.isOpen()) {
